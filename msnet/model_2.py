...
class MFP(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        # -----------------------------------------------
        self.leaky_relu_linear = torch.nn.Linear(2048, 1280)
        self.tanh_linear = torch.nn.Linear(2048,1280)
        self.sigmoid_linear = torch.nn.Linear(2048, 1280)
        self.softplus_linear = torch.nn.Linear(2048,1280)
        self.mish_linear = torch.nn.Linear(2048, 1280)
        self.selu_linear = torch.nn.Linear(2048,1280)
        self.elu_linear = torch.nn.Linear(2048, 1280)
        self.gelu_linear = torch.nn.Linear(2048,1280)
        self.kan = KAN([2*1280,1280,2*1280],spline_order=6,grid_size=10)
        # -----------------------------------------------
        self.linear_1 = torch.nn.Linear(8*1280,8*1024)
        self.linear_2 = torch.nn.Linear(8*1024,2*1280)
        self.linear_final_1 = torch.nn.Linear(2*1280,4)
        # -----------------------------------------------
        self.layer_norm_start = torch.nn.LayerNorm(2048,eps=1e-8)
        self.layer_norm_final = torch.nn.LayerNorm(8*1280,eps=1e-8)
...
class Model(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.mfp = MFP()
        self.model = AutoModelForCausalLM.from_pretrained("ai-forever/mGPT")

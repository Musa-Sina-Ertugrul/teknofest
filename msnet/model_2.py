...
class MFP(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        # -----------------------------------------------
        self.leaky_relu_linear = torch.nn.Linear(768, 512)
        self.tanh_linear = torch.nn.Linear(768,512)
        self.sigmoid_linear = torch.nn.Linear(768, 512)
        self.softplus_linear = torch.nn.Linear(768,512)
        self.mish_linear = torch.nn.Linear(768, 512)
        self.selu_linear = torch.nn.Linear(768,512)
        self.elu_linear = torch.nn.Linear(768, 512)
        self.gelu_linear = torch.nn.Linear(768,512)
        self.kan = KAN([4*512,2*512,4*512],spline_order=4,grid_size=10)
        # -----------------------------------------------
        self.linear_1 = torch.nn.Linear(8*512,6*512)
        self.linear_2 = torch.nn.Linear(6*512,4*512)
        self.linear_final_1 = torch.nn.Linear(4*512,1280)
        self.linear_final_2 = torch.nn.Linear(1280,4)
        # -----------------------------------------------
        self.layer_norm_start = torch.nn.LayerNorm(768,eps=1e-8)
        self.layer_norm_final = torch.nn.LayerNorm(8*512,eps=1e-8)
...
class Model(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.mfp = MFP()
        self.model = AutoModelForCausalLM.from_pretrained("FacebookAI/xlm-roberta-base")
        self.model.requires_grad_(True)
        self.model.lm_head.decoder = torch.nn.Flatten(start_dim=2)
